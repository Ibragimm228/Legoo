		...
		<header className='sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-200/50 shadow-sm'>
			<Container>
				<div className='flex items-center justify-between py-4'>
					...
					{/* Навигация справа - для всех экранов */}
							...
							{LINKS.map((item, index) => {
								...
								return (
									<Link
										key={item.label}
										to={item.href}
										className={`flex flex-col items-center px-1.5 py-1 text-xs font-medium text-gray-700 rounded-lg transition-all duration-200 ${mobileColors[index]} group min-w-0`}
									>
										{/* Иконки для мобильной версии */}
										<div className='w-3.5 h-3.5 mb-0.5 flex-shrink-0'>
											{item.label === 'Home' && (
												<svg fill='none' stroke='currentColor' viewBox='0 0 24 24'>
													<path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' />
												</svg>
										...
										<span className='text-center leading-tight truncate text-[10px]'>
											{item.label}
										</span>
									</Link>
								)
